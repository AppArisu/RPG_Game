...
void ImGui::SetWindowSize(ImGuiWindow* window, const ImVec2& size, ImGuiCond cond)
{
    ...
    IM_ASSERT(cond == 0 || ImIsPowerOfTwo(cond)); // ユーザーが複数の条件フラグを結合しようとしないようにしてください。
...
{
    IM_ASSERT(window->HitTestHoleSize.x == 0);     // 複数のホール/ヒットテストフィルタはサポートしていません。
...
// コンテンツサイズ=内側のスクロール可能な長方形で、WindowPaddingが埋め込まれています。
// SetNextWindowContentSize(ImVec2(100, 100) + ImGuiWindowFlags_AlwaysAutoResizeは、常に100x100アイテムの提出を許可します。
