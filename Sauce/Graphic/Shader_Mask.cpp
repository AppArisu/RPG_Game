...
ShaderMask::ShaderMask(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\Mask_VS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\Mask_PS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
	...
	// 定数バッファの生成
	{
		...
		HRESULT hr = device->CreateBuffer(&desc, 0, maskConstantBuffer.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
	}
}

// 描画開始
void ShaderMask::Begin(const RenderContext& rc)
{
...
// 描画
void ShaderMask::Draw(const RenderContext& rc, const Sprite* sprite)
{
	//	定数バッファの中身を更新します
	CbMask cbMask;
...
// 描画終了
void ShaderMask::End(const RenderContext& rc)
{
