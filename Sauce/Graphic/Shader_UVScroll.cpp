...
ShaderUVScroll::ShaderUVScroll(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\UVScroll_VS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\UVScroll_PS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void ShaderUVScroll::Begin(const RenderContext& rc)
{
...
// 描画
void ShaderUVScroll::Draw(const RenderContext& rc, const Sprite* sprite)
{
	CbScroll cbScroll;
...
// 描画終了
void ShaderUVScroll::End(const RenderContext& rc)
{
