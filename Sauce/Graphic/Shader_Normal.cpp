...
ShaderNormal::ShaderNormal(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\Sprite_VS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\Sprite_PS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void ShaderNormal::Begin(const RenderContext& rc)
{
...
// 描画
void ShaderNormal::Draw(const RenderContext& rc, const Sprite* sprite)
{
...
// 描画終了
void ShaderNormal::End(const RenderContext& rc)
{
