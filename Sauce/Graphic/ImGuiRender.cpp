	...
	: hWnd(hWnd)
{
	...
	ImFont* font = io.Fonts->AddFontFromFileTTF("Data\\Font\\NikkyouSans-mLKax.ttf", 18.0f, NULL, io.Fonts->GetGlyphRangesJapanese());
...
// 描画
void ImGuiRender::Render(ID3D11DeviceContext* context)
{
	ImGui::Render();
...
// マウス座標更新
void ImGuiRender::UpdateMousePos()
{
	ImGuiIO& io = ImGui::GetIO();
...
// WIN32メッセージハンドラー
LRESULT ImGuiRender::HandleMessage(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
...
// マウスカーソル更新
bool ImGuiRender::UpdateMouseCursor()
{
	ImGuiIO& io = ImGui::GetIO();
